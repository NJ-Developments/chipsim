...
let dealerPhase = false; // True when all players done, waiting for dealer results

// Default player template
function createPlayer(name, bankroll = 0) {
    return {
        name: name,
        bankroll: bankroll,
        bet: 0,
...
// Single player state (legacy - used when multiPlayerMode is false)
let bankroll = 0;
let bet = 0;
let lastBet = 0;
let status = '';
...
function toggleMultiPlayer() {
    if (!betConfirmed && !isDealing) {
        ...
        if (multiPlayerMode) {
            ...
            if (players.length === 0) {
                players = [
                    createPlayer('Player 1', 5),
                    createPlayer('Player 2', 5)
                ];
            }
            activePlayerIndex = 0;
            dealerPhase = false;
            // Disable two hands mode in multiplayer
            if (twoHandsMode) {
                bankroll += hand2Bet;
                hand2Bet = 0;
                twoHandsMode = false;
            ...
            // Switch back to single player - use first player's bankroll or existing
            if (players.length > 0 && players[0].bankroll > 0) {
        ...
        gameStarted = multiPlayerMode || bankroll > 0;
        saveToStorage();
        render();
    }
}
...
// Add a new player
function addPlayer() {
    if (players.length < 7 && !betConfirmed && !isDealing) {
        playSound('push');
        players.push(createPlayer('Player ' + (players.length + 1), 5));
        saveToStorage();
        render();
    }
}
...
function removePlayer(index) {
    if (players.length > 2 && !betConfirmed && !isDealing) {
        playSound('push');
...
// Add chips to a specific player
function addPlayerChips(index, amount) {
    if (players[index] && amount > 0) {
        playSound('push');
        players[index].bankroll += amount;
        saveToStorage();
        render();
    }
}

// Quick bet for a specific player
function mpQuickBet(playerIndex, amount) {
    const player = players[playerIndex];
    if (player && amount <= player.bankroll && !betConfirmed && !isDealing) {
        playSound('push');
        player.bet += amount;
...
// Clear bet for a specific player
function mpClearBet(playerIndex) {
    const player = players[playerIndex];
    if (player && player.bet > 0 && !betConfirmed && !isDealing) {
        playSound('push');
...
// Get dynamic bet amounts based on bankroll
function getMpBetAmounts(bankroll) {
    if (bankroll <= 0) return [];
    
    const amounts = [];
    
    if (bankroll >= 1 && bankroll < 10) {
        // Very small bankroll - $1 increments
        amounts.push(1);
        if (bankroll >= 2) amounts.push(2);
        if (bankroll >= 5) amounts.push(5);
    } else if (bankroll >= 10 && bankroll < 50) {
        // Small bankroll
        amounts.push(1, 5);
        if (bankroll >= 10) amounts.push(10);
        if (bankroll >= 25) amounts.push(25);
    } else if (bankroll >= 50 && bankroll < 200) {
        // Medium bankroll
        amounts.push(5, 10, 25);
        if (bankroll >= 50) amounts.push(50);
    } else if (bankroll >= 200 && bankroll < 500) {
        // Larger bankroll
        amounts.push(5, 25, 50, 100);
    } else if (bankroll >= 500) {
        // Large bankroll
        amounts.push(25, 50, 100, 200);
    }
    
    // Filter out amounts greater than bankroll
    return amounts.filter(a => a <= bankroll);
}

// Confirm all bets for multiplayer
function mpConfirmBets() {
    const hasAnyBet = players.some(p => p.bet > 0);
    if (hasAnyBet && !betConfirmed && !isDealing) {
        playSound('deal');
        betConfirmed = true;
        isDealing = true;
        ...
        players.forEach(p => {
            p.handComplete = false;
            p.status = '';
            p.winAmount = 0;
            p.isDoubleDown = false;
        ...
        activePlayerIndex = players.findIndex(p => p.bet > 0);
...
function mpNextPlayer() {
    // Mark current player as complete if they have a bet
    const currentPlayer = players[activePlayerIndex];
    if (currentPlayer && currentPlayer.bet > 0) {
        currentPlayer.handComplete = true;
    }
    
    // Find next player with incomplete hand and bet > 0
    let nextIndex = -1;
    for (let i = activePlayerIndex + 1; i < players.length; i++) {
        if (!players[i].handComplete && players[i].bet > 0) {
            nextIndex = i;
            break;
        }
    }
    ...
    if (nextIndex !== -1) {
        ...
        // All players done - enter dealer phase
        dealerPhase = true;
        render();
        return false;
    }
...
// Apply result to current player in multiplayer
function mpApplyResult(result) {
    const player = players[activePlayerIndex];
    if (!player || player.bet <= 0) return;
    
    const currentBet = player.isSplit ? 
        (player.activeHand === 1 ? player.bet : 
         player.activeHand === 2 ? player.splitBet : 
         player.activeHand === 3 ? player.splitBet2 : player.splitBet3) : 
        player.bet;
    
    if (player.isSplit) {
        // Handle split hand result
        const handKey = 'hand' + player.activeHand + 'Result';
        player[handKey] = result;
        
        // Calculate payout for this split hand
        let payout = 0;
        const betForHand = player.splitOriginalBet;
        
        if (result === 'win') {
            payout = betForHand * 2;
            player.stats.wins++;
            player.stats.totalWon += betForHand;
        } else if (result === 'blackjack') {
            payout = Math.floor(betForHand * 2.5);
            player.stats.blackjacks++;
            player.stats.wins++;
            player.stats.totalWon += Math.floor(betForHand * 1.5);
        } else if (result === 'push') {
            payout = betForHand;
            player.stats.pushes++;
        } else if (result === 'lose') {
            payout = 0;
            player.stats.losses++;
            player.stats.totalLost += betForHand;
        }
        
        player.bankroll += payout;
        player.winAmount += (result === 'win' || result === 'blackjack') ? payout - betForHand : 
                            (result === 'lose' ? -betForHand : 0);
        
        // Clear this hand's bet
        if (player.activeHand === 1) player.bet = 0;
        else if (player.activeHand === 2) player.splitBet = 0;
        else if (player.activeHand === 3) player.splitBet2 = 0;
        else player.splitBet3 = 0;
        
        // Move to next split hand or next player
        const nextHand = player.activeHand + 1;
        const maxHands = player.splitCount + 1;
        
        if (nextHand <= maxHands) {
            const nextBet = nextHand === 2 ? player.splitBet : 
                           nextHand === 3 ? player.splitBet2 : player.splitBet3;
            if (nextBet > 0) {
                player.activeHand = nextHand;
                saveToStorage();
                render();
                return;
            }
        ...
        // All split hands done
        player.handComplete = true;
        player.isSplit = false;
        player.status = player.winAmount > 0 ? 'win' : (player.winAmount < 0 ? 'lose' : 'push');
        
    } else {
        // Regular hand (no split)
        let payout = 0;
        ...
        if (result === 'win') {
            ...
            payout = betAmount * 2;
            player.stats.wins++;
            player.stats.totalWon += betAmount;
            if (betAmount > player.stats.biggestWin) player.stats.biggestWin = betAmount;
        } else if (result === 'blackjack') {
            playSound('blackjack');
            payout = Math.floor(betAmount * 2.5);
            player.stats.blackjacks++;
            player.stats.wins++;
            player.stats.totalWon += Math.floor(betAmount * 1.5);
            if (Math.floor(betAmount * 1.5) > player.stats.biggestWin) {
                player.stats.biggestWin = Math.floor(betAmount * 1.5);
            }
        } else if (result === 'push') {
            playSound('push');
            payout = betAmount;
            ...
            payout = 0;
            player.stats.losses++;
            player.stats.totalLost += betAmount;
        }
        
        player.bankroll += payout;
        player.winAmount = (result === 'win' || result === 'blackjack') ? payout - betAmount : 
    ...
    const allDone = players.every(p => p.handComplete || p.bet === 0);
    if (allDone) {
        mpEndRound();
    } else {
        mpNextPlayer();
...
// Double down for current player in multiplayer
function mpDoubleDown() {
    ...
    if (currentBet > 0 && currentBet <= player.bankroll && betConfirmed) {
        playSound('push');
        player.bankroll -= currentBet;
        
        if (player.isSplit) {
            if (player.activeHand === 1) { player.bet *= 2; player.split1DD = true; }
            else if (player.activeHand === 2) { player.splitBet *= 2; player.split2DD = true; }
            else if (player.activeHand === 3) { player.splitBet2 *= 2; player.split3DD = true; }
            else { player.splitBet3 *= 2; player.split4DD = true; }
        } else {
            player.bet *= 2;
...
// Split for current player in multiplayer
function mpSplit() {
    const player = players[activePlayerIndex];
    if (!player) return;
    ...
    if (currentBet > 0 && currentBet <= player.bankroll && betConfirmed) {
        if (!player.isSplit) {
            // First split
            playSound('push');
            player.splitOriginalBet = player.bet;
            player.isSplit = true;
            player.splitCount = 1;
            ...
            player.hand1Result = '';
            player.hand2Result = '';
        } else if (player.splitCount < 3) {
            // Additional splits
            playSound('push');
            player.splitCount++;
            if (player.splitCount === 2) {
                player.splitBet2 = currentBet;
                player.hand3Result = '';
            } else if (player.splitCount === 3) {
                player.splitBet3 = currentBet;
                player.hand4Result = '';
            }
            player.bankroll -= currentBet;
        }
...
// Surrender for current player in multiplayer
function mpSurrender() {
    const player = players[activePlayerIndex];
    if (!player || player.bet <= 0 || !betConfirmed || player.isSplit || player.isDoubleDown) return;
    
    playSound('push');
    const halfBet = Math.floor(player.bet / 2);
    const lostAmount = player.bet - halfBet;
    
    player.bankroll += halfBet;
    player.stats.totalLost += lostAmount;
    player.stats.losses++;
    player.stats.handsPlayed++;
    player.hasSurrendered = true;
    player.handComplete = true;
    player.status = 'surrender';
    player.winAmount = -lostAmount;
    player.bet = 0;
...
function render() {
    if (multiPlayerMode) {
        renderMultiPlayerTopStatus();
        renderMultiPlayerSide();
        renderMultiPlayerDealerSide();
    } else {
        renderTopStatusBar();
        renderPlayerSide();
        renderDealerSide();
...
// ==================== MULTI-PLAYER RENDER FUNCTIONS ====================

function renderMultiPlayerTopStatus() {
    ...
    if (!betConfirmed && !isDealing) {
        ...
        container.innerHTML = `
            <div class="top-status-container">
                <div class="betting-status ${hasAnyBet ? 'has-bet' : ''}">
                    ${hasAnyBet ? 'BETTING OPEN - Click Deal!' : 'Place Your Bets'}
                </div>
            </div>
        `;
    } else if (betConfirmed && !dealerPhase) {
        const player = players[activePlayerIndex];
        const playerName = player ? player.name : 'Player';
        container.innerHTML = `
            <div class="top-status-container">
                <div class="betting-status playing">
                    ${playerName}'s Turn - Select Outcome
                </div>
            </div>
        `;
    } else if (dealerPhase) {
...
function renderMultiPlayerSide() {
    const container = document.getElementById('playerSide');
    if (!container) return;
    ...
    players.forEach((player, index) => {
        ...
        const isComplete = player.handComplete;
        const hasBet = player.bet > 0 || (player.isSplit && (player.splitBet > 0 || player.splitBet2 > 0 || player.splitBet3 > 0));
        
        let statusClass = '';
        let resultDisplay = '';
        
        if (isComplete && player.status) {
            statusClass = 'player-' + player.status;
            const amountText = player.winAmount > 0 ? '+$' + player.winAmount : 
                              (player.winAmount < 0 ? '-$' + Math.abs(player.winAmount) : '$0');
            resultDisplay = `<div class="mp-result ${player.status}">${player.status.toUpperCase()} ${amountText}</div>`;
        }
        
        let betDisplay = '';
        if (!betConfirmed) {
            // Betting phase - show dynamic bet controls based on bankroll
            const bets = getMpBetAmounts(player.bankroll);
            betDisplay = `
                <div class="mp-bet-area">
                    <div class="mp-current-bet ${player.bet > 0 ? 'has-bet' : ''}">$${player.bet}</div>
                    <div class="mp-quick-bets">
                        ${bets.map(amt => `<button onclick="mpQuickBet(${index}, ${amt})" ${player.bankroll >= amt ? '' : 'disabled'}>+${amt}</button>`).join('')}
                        <button onclick="mpClearBet(${index})" ${player.bet > 0 ? '' : 'disabled'}>X</button>
                    </div>
                </div>
            `;
        } else if (!isComplete && hasBet) {
            // Playing phase - show current bet
            if (player.isSplit) {
                const handLabel = player.activeHand === 1 ? 'A' : (player.activeHand === 2 ? 'B' : (player.activeHand === 3 ? 'C' : 'D'));
                const currentBet = player.activeHand === 1 ? player.bet : 
                                   player.activeHand === 2 ? player.splitBet : 
                                   player.activeHand === 3 ? player.splitBet2 : player.splitBet3;
                betDisplay = `
                    <div class="mp-split-display">
                        <div class="mp-split-label">SPLIT Hand ${handLabel}</div>
                        <div class="mp-current-bet has-bet">$${currentBet}</div>
                    </div>
                `;
            } else {
                betDisplay = `
                    <div class="mp-current-bet has-bet ${player.isDoubleDown ? 'doubled' : ''}">
                        $${player.bet}${player.isDoubleDown ? ' (DD)' : ''}
                    </div>
                `;
            }
        }
        ...
        playersHTML += `
            <div class="mp-player-box ${isActive && betConfirmed ? 'active-player' : ''} ${statusClass}" onclick="setActivePlayer(${index})">
                <div class="mp-player-header">
                    <input type="text" class="mp-player-name" value="${player.name}" 
                           onchange="updatePlayerName(${index}, this.value)" 
                           onclick="event.stopPropagation()"
                           ${betConfirmed ? 'readonly' : ''}>
                    ${players.length > 2 && !betConfirmed ? `<button class="mp-remove-btn" onclick="event.stopPropagation(); removePlayer(${index})">X</button>` : ''}
                </div>
                <div class="mp-bankroll">$${player.bankroll}</div>
                ${betDisplay}
                ${resultDisplay}
            </div>
        `;
    ...
    // Add player button (if less than 7 and not in a round)
    let addPlayerBtn = '';
    if (players.length < 7 && !betConfirmed) {
        addPlayerBtn = `<button class="mp-add-player-btn" onclick="addPlayer()">+ Add Player</button>`;
    }
    ...
    if (!betConfirmed) {
        ...
        const hasAnyBet = totalBets > 0;
        dealBtn = `
            <div class="mp-deal-section">
                <button class="confirm-bet-btn" onclick="mpConfirmBets()" ${hasAnyBet ? '' : 'disabled'}>
                    DEAL CARDS - $${totalBets}
                </button>
            </div>
        `;
    }
    ...
    container.innerHTML = `
        <div class="panel-title player-title">PLAYERS</div>
        
        <div class="mp-toggle-section">
            <label class="toggle-label">
                <input type="checkbox" checked onchange="toggleMultiPlayer()">
                <span class="toggle-text">Multiplayer Mode (${players.length} players)</span>
            </label>
        </div>
...
function renderMultiPlayerDealerSide() {
    const container = document.getElementById('dealerSide');
    if (!container) return;
    
    const player = players[activePlayerIndex];
    const hasBet = player && (player.bet > 0 || (player.isSplit && (player.splitBet > 0 || player.splitBet2 > 0 || player.splitBet3 > 0)));
    const canAct = betConfirmed && player && !player.handComplete && hasBet;
    ...
    if (canAct) {
        ...
        const canDouble = currentBet > 0 && currentBet <= player.bankroll && !player.isDoubleDown;
        const canSplit = currentBet > 0 && currentBet <= player.bankroll && (!player.isSplit || player.splitCount < 3);
        const canSurrender = player.bet > 0 && !player.isSplit && !player.isDoubleDown && !player.hasSurrendered;
        
        actionButtons = `
            <div class="mp-player-actions">
                <div class="mp-action-label">Actions for ${player.name}:</div>
                <div class="mp-action-btns">
                    <button class="bet-action-btn btn-doubledown" onclick="mpDoubleDown()" ${canDouble ? '' : 'disabled'}>Double Down</button>
                    <button class="bet-action-btn btn-split" onclick="mpSplit()" ${canSplit ? '' : 'disabled'}>Split</button>
                    <button class="bet-action-btn btn-surrender" onclick="mpSurrender()" ${canSurrender ? '' : 'disabled'}>Surrender</button>
                </div>
            </div>
        `;
    }
    ...
    if (canAct) {
        resultButtons = `
            <div class="result-buttons">
                <button class="result-btn btn-win" onclick="mpApplyResult('win')">WIN</button>
                <button class="result-btn btn-blackjack" onclick="mpApplyResult('blackjack')">BLACKJACK</button>
                <button class="result-btn btn-push" onclick="mpApplyResult('push')">PUSH</button>
                <button class="result-btn btn-lose" onclick="mpApplyResult('lose')">LOSE</button>
            </div>
        `;
    }
...
// ==================== END MULTI-PLAYER RENDER FUNCTIONS ====================
...
function renderPlayerSide() {
    ...
    container.innerHTML = `
        ...
        <div class="two-hands-toggle">
            ...
            <label class="toggle-label" style="margin-left: 15px;">
                <input type="checkbox" onchange="toggleMultiPlayer()">
                <span class="toggle-text">Multiplayer</span>
            </label>
        </div>
        ` : ''}
